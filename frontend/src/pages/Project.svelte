...
<main>
    {#if project}
        ...
        <PageHeader
            options
            labels={views}
            baseHref={'/#/dashboard/projects/' + params.project_id}
            {params}
        >
            {project.name}
        </PageHeader>
        <div class="content">
            {#if params.page === null || params.page === 'resources'}
                ...
                <span class="subtitle"
                    >{project.budget_used} out of {project.budget} allocated {project.budget ===
                    1
                        ? 'cores'
                        : 'core'} used</span
                >
                {#if project.vms.length === 0}
                    <div class="empty-list">
                        ...
                        <Button href="/#/dashboard/create" style="margin-top: 20px;"
                            >CREATE VM</Button
                        >
                    </div>
                {:else}
